...
//	this._can = document.getElementById(opt.canvas);
//		console.log('INIT', opt.canvas, this._can, this._ctx, this);
...
GraphicEngine.prototype.clear = function() {
	...
	if(this._fade) {
		this._ctx.fillStyle = 'rgba(0,0,0,'+this._fadeFact+')';
		this._ctx.beginPath();
...
GraphicEngine.prototype.draw = function() {
	var self = this;
	self.clear();
//	self.graphics.circle({c:'blue', x:0, y:0, r:10});
	self._objects.forEach(
		function(obj) {
			if(isNaN(obj.x) || isNaN(obj.y)) {return;}


/*				self._ctx.fillStyle = obj.c;
			self._ctx.beginPath();
			self._ctx.arc(obj.x, obj.y, obj.r, 0, 2*_.PI, true);
			self._ctx.closePath();
			self._ctx.fill();*/
...
GraphicEngine.prototype.graphics.prototype = {
...
// Controls
//--------------------------------------------------------------------------------------------------
//GraphicEngine.prototype.controls = {
GraphicEngine.prototype.controls = function(s){ this._self = s; };

GraphicEngine.prototype.controls.prototype = {
	_self: null,

	//--------------------------------------------------------------------------------------------------
	refresh: function() { if(!this._self._isRun) { this._self.clear(); this._self.draw(); } },
	...
	//--------------------------------------------------------------------------------------------------
	zoomIn: function() { this._self._zoom = this._self._zoom<3 ? this._self._zoom+.1 : 3; this.refresh(); },

	//--------------------------------------------------------------------------------------------------
	zoomOut: function() { this._self._zoom = this._self._zoom>=.2 ? this._self._zoom-.1 : .1; this.refresh(); },

	//--------------------------------------------------------------------------------------------------
	moveUp: function() { this._self._offset.y += 50; this.refresh(); },

	//--------------------------------------------------------------------------------------------------
	moveDown: function() { this._self._offset.y -= 50; this.refresh(); },

	//--------------------------------------------------------------------------------------------------
	moveLeft: function() { this._self._offset.x += 50; this.refresh(); },

	//--------------------------------------------------------------------------------------------------
	moveRight: function() { this._self._offset.x -= 50; this.refresh(); },
